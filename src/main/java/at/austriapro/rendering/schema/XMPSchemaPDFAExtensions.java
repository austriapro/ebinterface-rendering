...
/**
 * Additionally to adding a RDF namespace with a indication which file attachment if Zugferd, this
 * namespace has to be described in a PDFA Extension Schema.
 *
 * I know there is a PDFAExtensionSchema in the context of PDFBox' XMPBOX but I have been using
 * PDFBox' JempBOX so far because I could not find out how to write XMPBOX XMPMetadata to a PDF
 * file.
 *
 * So this is my version of PDFAExtensionSchema for PDFBox' jempbox XMPMetadata
 *
 * @author jstaerk
 */
public class XMPSchemaPDFAExtensions extends XMPSchemaBasic {
  ...
  public XMPSchemaPDFAExtensions(org.apache.jempbox.xmp.XMPMetadata parent) {
    ...
    XMLUtil.setStringValue(nsInfoNode,
    ...
    addProperty(seqList1Node, "DocumentFileName", "Text", "external",
    ...
    addProperty(seqList2Node, "DocumentType", "Text", "external",
                //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                "INVOICE"); //$NON-NLS-1$
    ...
    addProperty(seqList3Node, "Version", "Text", "external",
                //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    addProperty(seqList4Node, "ConformanceLevel", "Text", "external",
                //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
